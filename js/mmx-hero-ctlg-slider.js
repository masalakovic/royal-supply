/**
 * MMX / HERO CTLG SLIDER
 */
class MMX_HeroCTLGSlider extends MMX_Element {
	#current_slide;
	#resize_observer;
	#auto_image_height;
	...
	themeResourcePattern = /mmx-(base|button|hero|hero-ctlg-slider)\/styles.css|family=Inter/i;
	...
	render() {
		return /*html*/`
			<div part="wrapper" class="mmx-hero-ctlg-slider mmx-hero-ctlg-slider--arrow-style-${this.getPropValue('arrow-style')} mmx-hero-ctlg-slider--per-page-${this.getCurrentPerPage()}">
				<div part="slides" class="mmx-hero-ctlg-slider__slides">
					${this.renderSlides()}
					<slot name="hero_slide"></slot>
				</div>
				<button class="mmx-hero-ctlg-slider__arrow-left" title="Move to previous slider page">${this.renderSliderArrowSVG()}</button>
				<button class="mmx-hero-ctlg-slider__arrow-right" title="Move to next slider page">${this.renderSliderArrowSVG()}</button>
				<div class="mmx-hero-ctlg-slider__slider-navigation mmx-hero-ctlg-slider__slider-navigation--position-${this.getPropValue('nav-position')} mmx-hero-ctlg-slider__slider-navigation--style-${this.getPropValue('nav-style')}"></div>
			</div>
		`;
	}
	...
	renderSliderArrowSVGCaret() {
		return `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="32.641" viewBox="0 0 20 32.641">
			<path class="mmx-hero-ctlg-slider__arrow-inner" d="M3.415,15.732l-.218.218.218.22L17.441,30.237l.01-.01,1.733-1.754-1.739-1.744L6.7,15.951,17.489,5.127l1.74-1.745L17.495,1.628l-.007-.007Z" transform="translate(0.771 0.391)" />
			<path class="mmx-hero-ctlg-slider__arrow-outer" d="M16.268,0,14.517,1.759.265,16.075,0,16.342H0L14.472,30.882l1.751,1.759.01-.01,1.98-2L4.186,16.56l-.218-.218.218-.218L18.259,2.012l-1.983-2Z" transform="translate(0 0)" />
		</svg>`;
	}

	renderSliderArrowSVGButton() {
		return `<svg xmlns="http://www.w3.org/2000/svg" width="7.197" height="11.831" viewBox="0 0 7.197 11.831">
			<path class="mmx-hero-ctlg-slider__arrow-button" d="M1.31,0,0,1.3,5.916,7.2l5.916-5.9L10.521,0,5.916,4.606Z" transform="translate(7.197) rotate(90)" />
		</svg>`;
	}
	...
	styles() {
		...
		return /*css*/`
			:host {
				--mmx-hero-ctlg-slider__per-page--mobile: ${perPageBreakpoints.mobile};
				--mmx-hero-ctlg-slider__per-page--tablet: ${perPageBreakpoints.tablet};
				--mmx-hero-ctlg-slider__per-page--desktop: ${perPageBreakpoints.desktop};
			}
		`;
	}
	...
	getPerPageBreakpoints() {
		...
		let current = this.sliderElement() ? window.getComputedStyle(this.sliderElement()).getPropertyValue('--mmx-hero-ctlg-slider__per-page') : mobile;
	...
	sliderElement() {
		return this.shadowRoot.querySelector('.mmx-hero-ctlg-slider');
	}
	...
	navigationLeftElement() {
		return this.shadowRoot.querySelector('.mmx-hero-ctlg-slider__arrow-left');
	}

	navigationRightElement() {
		return this.shadowRoot.querySelector('.mmx-hero-ctlg-slider__arrow-right');
	}

	navigationSliderElement() {
		return this.shadowRoot.querySelector('.mmx-hero-ctlg-slider__slider-navigation');
	}

	navigationSliderElements() {
		return this.shadowRoot.querySelectorAll('.mmx-hero-ctlg-slider__slider-navigation > button');
	}
...
if (!customElements.get('mmx-hero-ctlg-slider')) {
	customElements.define('mmx-hero-ctlg-slider', MMX_HeroCTLGSlider);
}
