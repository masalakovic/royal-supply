	...
	renderImageAsPicture() {
		...
		return /*html*/`
			<picture part="picture">
				${this.data?.image?.img?.responsive_images?.mobile ? /*html*/`<source class="source__mobile" media="(max-width: 640px)" srcset="${MMX.encodeEntities(MMX.encodeSrcset(this.data.image.img.responsive_images.mobile))}">` : ''}
				${this.data?.image?.img?.responsive_images?.tablet ? /*html*/`<source class="source__tablet" media="(max-width: 960px)" srcset="${MMX.encodeEntities(MMX.encodeSrcset(this.data.image.img.responsive_images.tablet))}">` : ''}
